...
;; hiccup version with fns
'(bling
  [:p.italic "myns.core:13:11"]
                            
  [:p.italic
   "The following symbol did not map to a publicly\n
    visible component rendering function:\n"
   [:bold x]
   [:br]
   [:bold.red (string/join (repeat (count (str x)) "^"))]]

  (p* "Component being demo'd:"
      (:fq-uic-name uic))

  (p* "Path, in the above function's metadata map:"
      [:opts (:opt-sym opt) :demo :samples i])

  (p* "Source snippet with problematic symbol:"
      (pprint-str coll 50)))


#_(print-bling
 [:italic "myns.core:13:11"]
 "\n\n"
 [:italic
  (str "The following symbol did not map to a publicly\n"
       "visible component rendering function:\n")]
 [:bold 'gold]
 "\n"
 [:bold.red (string/join (repeat (count "gold") "^"))]
 "\n\n"
 [:italic (str "hi" "\n")]
 [:bold "mono"])
...
(defn hydrate-bling-args [args]
  (let [problems (problem-args args)]
    (when (seq problems)
      (println "\nbad args")
      (pprint problems))
...
#_(apply 
 print-bling
 (hydrate-bling-args
  [[:p.italic "myns.core:13:11" [:bold.red " Hi"] "this should be italic"]
   [:p
    (str "The following symbol did not map to a publicly\n"
         "visible component rendering function:\n")
    [:bold 'gold]
